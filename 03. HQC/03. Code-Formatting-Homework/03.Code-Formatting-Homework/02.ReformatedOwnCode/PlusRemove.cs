...
namespace _02.ReformatedOwnCode
{
    class Program
    {
        static void Main()
        {
            ...
            for (int row = 1; row < matrix.Count - 1; row++)
            {
                ...
                {
                    curVal = Char.ToLower(matrix[row][col]);

                    if (col < matrix[row - 1].Count() && curVal.Equals(char.ToLower(matrix[row - 1][col])) &&
                        curVal.Equals(char.ToLower(matrix[row][col - 1])) && curVal.Equals(char.ToLower(matrix[row][col + 1]))
                        && col < matrix[row + 1].Count() && curVal.Equals(char.ToLower(matrix[row + 1][col])))
                    {
