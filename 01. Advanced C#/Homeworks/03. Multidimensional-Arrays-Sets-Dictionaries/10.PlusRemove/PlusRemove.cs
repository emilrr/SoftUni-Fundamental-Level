        ...
        static void Main()
        {
            ...
            for (int row = 1; row < matrix.Count - 1; row++)
            {
                for (int col = 1; col < matrix[row].Count() - 1; col++)
                {
                    ...
                    if (col < matrix[row - 1].Count() && currentValue.Equals(char.ToLower(matrix[row - 1][col])) &&
                        currentValue.Equals(char.ToLower(matrix[row][col - 1])) && currentValue.Equals(char.ToLower(matrix[row][col + 1]))
                        && col < matrix[row + 1].Count() && currentValue.Equals(char.ToLower(matrix[row + 1][col])))
                    {
